...
def create_freq_chart(variable):
    ...
    final_games_df = new_games_df.head(20)
    freq_chart = alt.Chart(final_games_df).mark_bar().encode(
...
def create_scatter_plot(sales_selection, score_selection):
    ...
    scatter = alt.Chart(final_games_df).mark_circle(size=60,opacity=0.4).encode(
            x=score_selection,
            y=sales_selection,
...
with tab1:
    ...
    selection=st.selectbox("Choose an Attribute",['','Console','Genre','Publisher','Developer'])
